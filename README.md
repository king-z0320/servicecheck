...
知识类别可以是 `RULE`、`POLICY`、`GOOD_CASE`、`BAD_CASE` 或 `BOUNDARY_CASE`；案例不会仅因类别被排除，但如果没有明确规则来源关系，仍不能单独支持处罚。`BOUNDARY_CASE` 用于保存相似但不能自动下结论的边界场景。
...
- negativeMax：`0.695651`；
- indexVersion：`97ad4773f89c`；
- 12 条正例和 10 条阈值负例参与校准；2 条高语义相似边界挑战样例单独保留，不用于拟合单一阈值。
...
- 事件枚举有九类，`DirectAnalyzer` 已对九类事件提供确定性违规判定：前三类沿用业务审计联动，其余六类按客户事件与坐席后续违规话术组合判定；
- 情绪识别主要用于展示，不参与权威扣分；
