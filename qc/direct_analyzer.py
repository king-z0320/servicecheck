...
class DirectAnalyzer:
    ...
    def analyze(self, request: AnalysisRequest) -> QualityReport:
        ...
        events = self.extractor.extract(request.transcript)
        ...
        unique_hits = {h.documentId: h for h in hits}
        ...
        if any(event.ambiguous for event in events) or audit.errors:
    ...
    def _check_repayment_dispute(self, event, transcript, turn_by_id, audit):
        ...
        return Violation(
            ...
            knowledgeDocumentIds=[rule.sourceDocumentId],
            explanation=(
...
def requires_loop(
    ...
    if not report.knowledgeHits and report.violations:
